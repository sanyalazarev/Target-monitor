...
function loadData() {
	...
	dataSocket.emit('getLeads', JSON.stringify(req));
	
	$(".preloader").show();
}

dataSocket.on('getLeads', function (data) {
	...
	if(data.leads.length) {
		...
		$("#list .table-striped tr:last").after('<tr><td class="success text-center" colspan="9">END OF LIST REACHED</td></tr>');
});
...
function updateList(data) {
	...
	<td>' + data.leads[i].affiliateName + '</td>\
	<td>' + (data.leads[i].subAffiliateName.length ? data.leads[i].subAffiliateName.join(", ") : 'NO') + '</td>\
	<td>' + data.leads[i].creativeName + '</td>\
	<td>' + data.leads[i].country + '</td>\
	<td>' + data.leads[i].reward + '$</td>\
	<td>' + data.leads[i].events.join(", ") + '</td>\
