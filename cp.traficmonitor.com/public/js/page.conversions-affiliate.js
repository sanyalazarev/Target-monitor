...
function loadData() {
	...
	dataSocket.emit('getConversionsAffiliate', JSON.stringify(req));
	
	$(".preloader").show();
}

dataSocket.on('getConversionsAffiliate', function (data) {
	...
	if(data.conversions.length) {
		...
		$("#list .table-striped tr:last").after('<tr><td class="success text-center" colspan="17">END OF LIST REACHED</td></tr>');
});
