...
function loadData() {
	...
	dataSocket.emit('getClicksAffiliate', JSON.stringify(req));
	
	$(".preloader").show();
}

dataSocket.on('getClicksAffiliate', function (data) {
	...
	if(data.clicks.length) {
		...
		$("#list .table-striped tr:last").after('<tr><td class="success text-center" colspan="13">END OF LIST REACHED</td></tr>');
});
